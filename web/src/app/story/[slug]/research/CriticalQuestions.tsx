...
export function CriticalQuestions({ items, className = "" }: CriticalQuestionsProps) {
  ...
  return (
    <ul className={`list-disc space-y-3 pl-5 text-sm leading-relaxed text-gray-900 marker:text-gray-700 dark:text-gray-100 dark:marker:text-gray-500 ${className}`.trim()}>
      {list.map((q, i) => (
