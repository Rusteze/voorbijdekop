  ...
  for (const story of stories) {
    ...
    try {
      ...
      const outputFormatBlock = [
        ...
        "- Claims: formuleer concrete, verifieerbare beweringen + verificatiemethode die verwijst naar welke bron(s) je zou openen (gebruik de exact gegeven `sourceDomain` uit sourcesPayload).",
        "",
        "TAAL (MUST):",
        ...
        "  Wat hier opvalt",
        "  gevolgd door minstens **twee** alinea's: wat opvalt qua inconsistentie, framing, ontbrekend bewijs of onduidelijkheid.",
        "",
        "Onderzoek dit verhaal (investigations) — uitsluitend OSINT:",
        "- Alles is **open-source intelligence (OSINT)**: alleen vrij toegankelijke, legale bronnen en tools (web, kaarten, archieven, open registers, sociale media in het openbaar).",
        "- Geen betaalde databanken, geen hacking, geen toegang tot niet-publieke systemen, geen instructies die privacy of wet overtreden.",
        "- Maak suggesties concreet. Geen 'zoek meer info'.",
        "- Per onderzoek: noem precies welke claim/actor/locatie/tijd je wil toetsen en welke observatie de uitkomst bevestigt of ontkracht.",
        "- Per investigation: ten minste één stap of tool moet expliciet verwijzen naar een concrete bron uit sourcesPayload (exacte `sourceDomain`, bv. `nos.nl`).",
        "- `steps`: schrijf per stap in de vorm 'OSINT-actie — check bij {sourceDomain} of met een openbare methode (archief, kaart, register): {wat je ziet/meet}'.",
        "",
